...
class FactsViewModel: ObservableObject {
    ...
    init(category: AnimalCategory, imageLoader: ImageLoader) {
        ...
        let allFactItems: [FactDisplayItem] = category.content.enumerated().map { (idx, singleContent) in
            let previousEnabled = idx > 0
            let nextEnabled = idx < category.content.count - 1
